...
function Qlearning(Q, policy, num_episodes, α; plotting=true, target_update_interval=100)
    ...
    for i = 1:num_episodes
        ...
        for (s,a,r,s1) in ep # An episode object is iterable
            Q[s,a] = @show r + γ*max_a(Q, s1) # Update Q using Q-target
...
ho = Hyperopt.@phyperopt for i=80, sampler = RandomSampler(),
    # α   = LinRange(0.5,3,200),
    λ   = exp10.(LinRange(-6,0,200)),
    tui = round.(Int, exp10.(LinRange(1, 1.5, 200)))
    # P0  = exp10.(LinRange(0, 15, 200))

    @show i
    α, λ, tui, P0 = 1, 0.9999, 40, 1.5e10
    ...
    gridm        = Grid(domain, m, splitter, initial_split=4:5)
